...
namespace Jungo.usb_lib
{
    ...
    public class UsbPipe
    {
            ...
            DWORD timeOut)
        {
            ...
            m_dwTransferStatus = wdu_lib_decl.WDU_Transfer(m_hDev,
                m_pPipe.dwNumber, (DWORD)(fRead == true ? 1 : 0), dwOptions,
                buffer, dwBuffSize, ref dwBytesTransfered, pSetupPacket,
                timeOut);
            ...
            for (dwPipeIndex = 0; dwPipeIndex < dwNumOfPipes; ++dwPipeIndex)
            {
                ...
                pipe_info = (WDU_PIPE_INFO)Convert.ChangeType
                    (Marshal.PtrToStructure(new
                        IntPtr((int)((DWORD)pActiveAltSetting.pPipes.ToInt64()
    ...
    public class UsbDevice
    {
        ...
        private WORD wVid;
        private WORD wPid;
        private DWORD dwInterfaceNum; // Interface number currently used by the
                                      // app
        ...
        {
            for (DWORD i = 0; i < dwNumOfInterfaces; ++i)
            {
            ...
            DWORD newSetting)
        {
            ...
            if (newInterface == this.dwInterfaceNum && newSetting ==
                this.dwAltSettingNum)
            {
                return dwStatus;
            }
            ...
            WDU_ALTERNATE_SETTING pActiveAltSetting =
                (WDU_ALTERNATE_SETTING)Marshal.PtrToStructure(
                    interfacesInfo[index].wduInterface.pActiveAltSetting,
                    typeof(WDU_ALTERNATE_SETTING));
